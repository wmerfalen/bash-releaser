...
curl -v https://uploads.github.com/repos/${GITHUB_USER}/${GITHUB_REPO}/releases/${GITHUB_RELEASE_ID}/assets -G --data-urlencode name@$PWD/.tmp/__tmp 2>&1 | grep -E '^> GET /' > haxx
cat haxx | sed -E 's|^> GET ||' | cut -d' ' -f1 > $PWD/.tmp/url
rm haxx
